...
time_t st_orphanage = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());

std::string out_orphan_tx_path = "../../../bitcoin_mempool_data/hyw/out_orphan_tx/";
std::string in_orphan_tx_path = "../../../bitcoin_mempool_data/hyw/in_orphan_tx/";

FILE* in_orphan_tx = fsbridge::fopen(
    fs::PathFromString(in_orphan_tx_path + fs::timetodate(st_orphanage) + ".json"),
    "a"
);
FILE* out_orphan_tx = fsbridge::fopen(
    fs::PathFromString(out_orphan_tx_path + fs::timetodate(st_orphanage) + ".json"),
    "a"
);
...
bool TxOrphanage::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    if(now_time - st_orphanage > 86400 || !out_orphan_tx){
        fclose(in_orphan_tx);
        st_orphanage = now_time;
        in_orphan_tx = fsbridge::fopen(
            fs::PathFromString(in_orphan_tx_path + fs::timetodate(st_orphanage) + ".json"), 
            "a"
        );
    }
    ...
    for (const CTxIn& txin : ctx.vin){
        ...
        vinstr = vinstr + 
                ...
                "\"txinwitness\": " + "\"\"" +                     
                "}";
        if(cnt != ctx.vin.size()) vinstr += ",";
    }
    vinstr += "]";
    std::string voutstr = "[";
    int voutn = 0;
    cnt = 0;            
    for (const CTxOut& txout : ctx.vout){
        cnt++;
        voutstr = voutstr + 
                ...
                "\"scriptPubKey\": " + "\"\"" +               
                "}";
        if(cnt != ctx.vout.size()) voutstr += ",";
    }
    voutstr += "]";
    
    std::string writeStr = "{\"txid\": \"" + tx->GetHash().GetHex() + "\"" + "," + 
                            "\"hash\": " + "\"" + tx->GetWitnessHash().GetHex() + "\"" + "," + 
                            "\"version\": " + "\"" + std::to_string(ctx.nVersion) + "\"" + "," +  
                            "\"enterTime\": " + "\"" + fs::timet_to_str(now_time) + "\"" + "," + 
                            "\"size\": " + "\"" + std::to_string(tx->GetTotalSize()) + "\"" + "," + 
                            "\"vin\": " + vinstr + "," + 
                            "\"vout\": " + voutstr + 
                            "}\n";

    cuz_write_file(writeStr, in_orphan_tx);
...
int TxOrphanage::EraseTx(const uint256& txid)
{
    ...
    // out orphan
    LogPrintf("EraseTx() 孤儿\n");
    time_t now_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    if(now_time - st_orphanage > 86400 || !out_orphan_tx){
        fclose(out_orphan_tx);
        st_orphanage = now_time;
        out_orphan_tx = fsbridge::fopen(
            fs::PathFromString(out_orphan_tx_path + fs::timetodate(st_orphanage) + ".json"), 
            "a"
        );
    }
    ...
    std::string writeStr = "{\"txid\": \"" + tx->GetHash().GetHex() + "\"" + "," + 
                            ...
                            "\"vout\": " + voutstr + 
                            "}\n";
    cuz_write_file(writeStr, out_orphan_tx);
    return 1;
}
