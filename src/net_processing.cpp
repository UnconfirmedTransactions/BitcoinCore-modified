...
time_t st_net_process = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());

std::string notaccpet_by_mempool_tx_path = "../../../bitcoin_mempool_data/hyw/notaccpet_by_mempool_tx/";
std::string enter_peernode_path ="../../../bitcoin_mempool_data/hyw/enter_peernode/";
std::string inmempool_tx_path = "../../../bitcoin_mempool_data/hyw/inmempool_tx/";

// 过滤掉的tx
FILE* notaccpet_by_mempool_tx = fsbridge::fopen(
    fs::PathFromString(notaccpet_by_mempool_tx_path + fs::timetodate(st_net_process) + ".json"), 
    "a"
);
// 进peernode的所有tx
FILE* enter_peernode = fsbridge::fopen(
    fs::PathFromString(enter_peernode_path + fs::timetodate(st_net_process) + ".json"), 
    "a"
);
// 进交易池的tx
FILE* inmempool_tx = fsbridge::fopen(
    fs::PathFromString(inmempool_tx_path + fs::timetodate(st_net_process) + ".json"), 
    "a"
);

static int cuz_write_file(const std::string &str, FILE *fp)
{
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERACK) {
        ...
        if (!pfrom.IsInboundConn()) {
            ...
            LogPrintf("Newhahaha outbound peer connected: version: %d, blocks=%d, peer=%d%s (%s)\n",
                      pfrom.nVersion.load(), peer->m_starting_height,
                      pfrom.GetId(), (fLogIPs ? strprintf(", peeraddr=%s", pfrom.addr.ToString()) : ""),
                      pfrom.ConnectionTypeAsString());
            LogPrintf("connected to the address at %s, \n", pfrom.addr.ToString());
	    //            LogPrintf(pfrom.addrBind.ToString())
        }
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        for (const CTxIn& txin : tx.vin){
            // for(auto t : txin.scriptSig){
                std::cout << txin.ToString() << std::endl;
            // }
            cnt++;
            vinstr = vinstr + 
                    "{" + "\"txid\": " + "\"" + txin.prevout.hash.ToString() + "\"" + "," +
                    "\"vout\": " + "\"" + std::to_string(txin.prevout.n) + "\"" + "," +
                    "\"scriptSig\": " + "\"" + "\"," +
                    "\"sequence\": " + "\"" + std::to_string(txin.nSequence) + "\"" + "," + 
                    "\"txinwitness\": " + "\"\"" +                     
                    "}";
            if(cnt != tx.vin.size()) vinstr += ",";
        }
        vinstr += "]";
        ...
        for (const CTxOut& txout : tx.vout){
            cnt++;
            voutstr = voutstr + 
                    "{" + "\"value\": " + "\"" + std::to_string(txout.nValue) + "\"" + "," +
                    "\"n\": " + "\"" + std::to_string(voutn++) + "\"" + "," +
                    "\"scriptPubKey\": " + "\"\"" +               
                    "}";
            if(cnt != tx.vout.size()) voutstr += ",";
        }
        voutstr += "]";
        
        std::string writeStr = "{\"txid\": \"" + ptx->GetHash().GetHex() + "\"" + "," + 
                                "\"hash\": " + "\"" + ptx->GetWitnessHash().GetHex() + "\"" + "," + 
                                "\"version\": " + "\"" + std::to_string(pfrom.nVersion) + "\"" + "," +  
                                "\"enterTime\": " + "\"" + fs::timet_to_str(now_time) + "\"" + "," + 
                                "\"mempoolip\": " + "\"" + pfrom.addr.ToString() + "\"" + ","
                                "\"totalSize\": " + "\"" + std::to_string(ptx->GetTotalSize()) + "\"" + "," + 
                                "\"vin\": " + vinstr + "," + 
                                "\"vout\": " + voutstr + 
                                "}\n";
        LogPrintf("ProcessMessage() TX, 进入节点的所有tx\n");
        LogPrintf("%i", writeStr);
        
        if(now_time - st_net_process > 86400 || !enter_peernode){
            fclose(enter_peernode);
            st_net_process = now_time;
            enter_peernode = fsbridge::fopen(
                fs::PathFromString(enter_peernode_path + fs::timetodate(st_net_process) + ".json"), 
                "a"
            );
        }
        cuz_write_file(writeStr, enter_peernode);  
        
        // tx开始进池
        const MempoolAcceptResult result = m_chainman.ProcessTransaction(pfrom.addr.ToString(), ptx);
        const TxValidationState& state = result.m_state;
        auto it = m_mempool.GetIter(txid).value();
        writeStr = "{\"txid\": \"" + ptx->GetHash().GetHex() + "\"" + "," + 
                                "\"hash\": " + "\"" + ptx->GetWitnessHash().GetHex() + "\"" + "," + 
                                "\"version\": " + "\"" + std::to_string(pfrom.nVersion) + "\"" + "," +  
                                "\"enterTime\": " + "\"" + fs::timet_to_str(now_time) + "\"" + "," + 
                                "\"enterHeight\": " + "\"" + std::to_string(it->GetHeight()) + "\"" + "," + 
                                "\"mempoolip\": " + "\"" + it->GetMempoolIp() + "\"" + ","
                                "\"size\": " + "\"" + std::to_string(it->GetTx().GetTotalSize()) + "\"" + "," + 
                                "\"vsize\": " + "\"" + std::to_string(GetInfo(it).vsize) + "\"" + "," + 
                                "\"weight\": " + "\"" + std::to_string(it->GetTxWeight()) + "\"" + "," + 
                                "\"descendantsize\": " + "\"" + std::to_string(it->GetSizeWithDescendants()) + "\"" + "," + 
                                "\"descendantcount\": " + "\"" + std::to_string(it->GetCountWithDescendants()) + "\"" + "," + 
                                "\"ancestorsize\": " + "\"" + std::to_string(it->GetSizeWithAncestors()) + "\"" + "," + 
                                "\"ancestorcount\": " + "\"" + std::to_string(it->GetCountWithAncestors()) + "\"" + "," + 
                                "\"isunbroadcast\": " + "\"" + std::to_string(m_mempool.IsUnbroadcastTx(it->GetTx().GetHash())) + "\"" + "," + 
                                "\"fees\": {\"base\": " + "\"" + std::to_string(it->GetFee()) + "\"" + "," + 
                                            "\"modified\": " + "\"" + std::to_string(it->GetModifiedFee()) + "\"" + "," + 
                                            "\"ancestor\": " + "\"" + std::to_string(it->GetModFeesWithAncestors()) + "\"" + "," + 
                                            "\"descendant\": " + "\"" + std::to_string(it->GetModFeesWithDescendants()) + "\"" + 
                                            "}" + "," + 
                                "\"vin\": " + vinstr + "," + 
                                "\"vout\": " + voutstr + "," + 
                                "\"state\": " + "\"" + state.ToString()  + "\"" + 
                                "\"stateReslut:\"" + "\"" + std::to_string(static_cast<unsigned int>(state.GetResult())) + "\"" + 
                                "}\n";
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            // accpet的tx
            time_t now_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
            if(now_time - st_net_process > 86400 || !inmempool_tx){
                fclose(inmempool_tx);
                st_net_process = now_time;
                inmempool_tx = fsbridge::fopen(
                    fs::PathFromString(inmempool_tx_path + fs::timetodate(st_net_process) + ".json"), 
                    "a"
                );
            }
            LogPrintf("ProcessMessage() 通过的 valid, inmempool_tx\n");
            cuz_write_file(writeStr, inmempool_tx);  
        ...
        if (state.IsInvalid()) {
            // notaccpect to txmempool
            LogPrintf("ProcessMessage() IsInvalid, 不合法的tx\n");
            LogPrintf("%i", writeStr);
            time_t now_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
            if(now_time - st_net_process > 86400 || !notaccpet_by_mempool_tx){
                fclose(notaccpet_by_mempool_tx);
                st_net_process = now_time;
                notaccpet_by_mempool_tx = fsbridge::fopen(
                    fs::PathFromString(notaccpet_by_mempool_tx_path + fs::timetodate(st_net_process) + ".json"), 
                    "a"
                );
            }
            cuz_write_file(writeStr, notaccpet_by_mempool_tx);  
...
void PeerManagerImpl::MaybeSendAddr(CNode& node, Peer& peer, std::chrono::microseconds current_time)
{
   ...
   // LogPrint("ip=%d currenttime=%d delay_time=%d\n",
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
       ...
       //                  LogPrintf("peer=%s: accepted %s (poolsz %u txn, %u kB)\n",pfrom.addr.ToString(),tx.GetHash().ToString(),m_mempool.size(), m_mempool.DynamicMemoryUsage() / 1000);
//		           LogPrintf("target_ip=%s;current_time=%u;relay_time=%u\n",pto->addr.ToString(),current_time.count(),tx_relay->m_next_inv_send_time.count());
//			   LogPrintf("target_ip=%s;current_time=%u",pto->addr.ToString(),current_time.count());
		    } else {
